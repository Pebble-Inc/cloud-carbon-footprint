 ...
 */
export async function appendToInlinePolicy(
  tenantAccountId: string,
  envVal: string,
  ccfTenentRole: string,
): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      existingPolicyDocument = JSON.parse(
    ...
    let statement = existingPolicyDocument.Statement.find(
    ...
    console.log(
      `✅ Successfully updated policy '${policyName}' for role '${role}'`,
    )
  } catch (error) {
