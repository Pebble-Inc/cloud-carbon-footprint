...
export default class DocumentDbCacheManager extends CacheManager {
  ...
  /**
   * Creates a connection to the AWS DocumentDB using the provided URI and credentials.
   * This function needs to be invoked before using this DocumentDbCacheManager.
   * @returns {Promise<void>} A Promise that resolves when the connection is established.
   */
  static async createDbConnection() {
    const logger = new Logger('DocumentDB')
    const {
      DOCUMENTDB: { URI, SSL_CA_FILE, USERNAME, PASSWORD },
    } = configLoader()

    if (!URI) {
      logger.error(
        'There was an error connecting to DocumentDB',
    ...
    try {
      // Configure connection options for DocumentDB
      const options: any = {
        useNewUrlParser: true,
        useUnifiedTopology: true,
      }

      // If SSL CA file is provided, configure SSL
      if (SSL_CA_FILE) {
        options.ssl = true
        options.sslValidate = true
        options.sslCA = SSL_CA_FILE
      }
      ...
      DocumentDbCacheManager.mongoClient = new MongoClient(URI, options)
      await DocumentDbCacheManager.mongoClient.connect()
      logger.info('Successfully connected to DocumentDB')
    } catch (error) {
      logger.error('There was an error connecting to DocumentDB', error)
    }
  }
  ...
  async getEstimates(
    ...
    try {
      ...
      const database = DocumentDbCacheManager.mongoClient.db(this.dbName)
      ...
      this.cacheLogger.warn(
        `There was an error getting estimates from DocumentDB: ${e.message}`,
      )
    }
  ...
  async setEstimates(
    ...
    try {
      ...
      this.cacheLogger.warn(
        `There was an error setting estimates in DocumentDB: ${e.message}`,
      )
    }
  }
  ...
  async getMissingDates(
    ...
    this.cacheLogger.info('Using DocumentDB database...')
    ...
    try {
      ...
      this.cacheLogger.warn(
        `There was an error getting missing dates from DocumentDB: ${e.message}`,
      )
      return []
    }
  }
